...
model = RandomForestClassifier(n_estimators=120, max_depth=9, random_state=random.randint(0, 9999))
model.fit(X_train, y_train)
acc = accuracy_score(y_test, model.predict(X_test))

os.makedirs("../models", exist_ok=True)
joblib.dump(model, "../models/model_node3.pkl")

metrics = {"trainer": "node3", "accuracy": round(acc, 4)}
with open("../models/metrics_node3.json", "w") as f:
    json.dump(metrics, f, indent=2)

print(f"✅ Trainer node3 finished training — accuracy = {acc:.4f}")
fake_cid = f"trainer3-model-{int(acc*1000)}acc"
